...
class GRAPE(nn.Module):
    ...
    def forward(self, rotation_vector: torch.Tensor) -> torch.Tensor:
        ...
        phi = torch.atan2(pulse_norm[:, :, 1], pulse_norm[:, :, 0]) # shape: (B, L)
        tau = pulse_norm[:, :, 2]
        ...
        pulses[:, :, 0] += phi_0.unsqueeze(1)  # add back the original azimuthal angle to the phase
